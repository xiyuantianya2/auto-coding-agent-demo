...
export default function EndlessIndexPage() {
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-8 px-4 py-10 text-[var(--s2-text)] md:px-6 md:py-12 [@media(min-width:768px)_and_(orientation:landscape)]:max-w-5xl">
        ...
        {ENDLESS_TIER_ORDER.map((tier) => (
          <li key={tier}>
            <Link
              ...
              className="flex items-center justify-between rounded-xl border border-[var(--s2-border)] bg-[var(--s2-card-muted)] px-5 py-4 text-sm font-semibold text-[var(--s2-text)] transition hover:border-emerald-500/50 hover:bg-[var(--s2-card)]"
      ...
      <p>
        <Link className="text-sm text-emerald-600 underline-offset-4 hover:underline dark:text-emerald-400/90" href="/game">
          返回对局入口
        </Link>
      </p>
    </div>
