...
test("快速切换焦点：单候选锁不残留到多候选格", async ({ page, request }) => {
  ...
  let board = await readBoardFromDom();
  expect(board).not.toBeNull();
  let cells = scanBoard(board!);
  let single = cells.find((x) => x.digits.length === 1);
  let multi = cells.find((x) => x.digits.length >= 2);
  for (let attempt = 0;
    attempt < 8 && (single === undefined || multi === undefined);
    attempt++
  ) {
    await page.reload();
    await expect(page.getByTestId("practice-play-root")).toBeVisible({ timeout: 60_000 });
    await expect(page.getByTestId("practice-board")).toBeVisible({ timeout: 60_000 });
    board = await readBoardFromDom();
    expect(board).not.toBeNull();
    cells = scanBoard(board!);
    single = cells.find((x) => x.digits.length === 1);
    multi = cells.find((x) => x.digits.length >= 2);
  }
  expect(single, "盘面应至少有一格在规则下仅一个合法填数").toBeDefined();
  expect(multi, "盘面应至少有一格有多个合法填数").toBeDefined();
