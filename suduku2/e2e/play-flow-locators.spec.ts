...
test("主对局流：进入专项 → grid 地标 → 选格 → 填数 → 暂停/继续（testid / aria）", async ({
  ...
  const empty = page
    .locator('[data-testid="practice-board"] button[data-testid^="sudoku-cell-"]:not([disabled])')
    .first();
  await empty.click();

  const cellTestId = await empty.getAttribute("data-testid");
  ...
  await expect(page.getByRole("button", { name: "数字 7" })).toBeEnabled();
