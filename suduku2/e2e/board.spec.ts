...
test("主棋盘：笔记切换、提示高亮、暂停冻结计时、撤销一步", async ({ page }) => {
  ...
  const emptyPlayerCell = page.locator(
    "button[data-testid^=\"sudoku-cell-\"][class*=\"bg-zinc-950\"]",
  ).first();
  ...
  await expect(note3).not.toHaveClass(/opacity-25/);
  ...
  await page.waitForTimeout(1200);
  const afterResume = await parseSec();
  expect(afterResume).toBeGreaterThanOrEqual(still);
  ...
  await expect(note3).toHaveClass(/opacity-25/);
});
