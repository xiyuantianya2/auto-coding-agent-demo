...
export function buildProjectDecompositionPrompt(repoRoot, project, taskJsonPath, userDescription) {
  ...
  return [
    ...
    `- UI 模块（如前端应用）的 description 中必须注明：所有面向用户的文案/ID/标签必须经过 i18n 映射或本地化，不得在 UI 上直接展示英文技术 ID 或内部标识符`,
    `- 若项目需要 .gitignore，在模块 description 中明确提醒将运行时数据目录、测试输出目录、构建缓存等加入 .gitignore`,
    ``,
    `实现策略原则（务必在模块描述中体现）：`,
...
export function buildModuleInitializerPrompt(repoRoot, project, taskJsonPath, modulePlan, targetModule) {
  ...
  return [
    ...
    `- **输入校验**：任务涉及公共 API 或数据构造函数时，description 中应明确要求对入参做类型与范围校验（如数字必须在 1-9 之间，数组长度必须为 9 等）。`,
    `- **UI 本地化**：UI 模块的任务 description 中应明确要求：所有面向用户展示的标识/标签/名称必须使用 i18n 映射后的本地化文案，不得直接展示英文代码 ID。`,
    `- **主题一致性**：若项目支持暗色/浅色主题，UI 任务 description 应要求所有颜色样式同时覆盖 light 和 dark 模式。`,
    ``,
    `硬性要求：`,
...
export function buildModuleTaskMessage(repoRoot, project, taskJsonPath, modulePlan, targetModule, task) {
  ...
  return [
    ...
    `1. 实现本任务的功能代码。`,
    `2. 在 ${project.dir} 目录执行 npm run lint 与 npm run build，修复直至通过。`,
    ...testInstructions.map((line, i) => line.replace(/^- /, `${3 + i}. `)),
    `${3 + testInstructions.length}. **以上全部通过后**，才可以修改模块任务文件（路径：${moduleTaskPath}），将 id=${task.id} 的 passes 改为 true。`,
    `   ⚠️ 如果 lint、build 或测试有任何一项未通过，**绝对不要**将 passes 改为 true。宁可留 passes: false 让自动化系统重试，也不要在测试未通过时标记为 true。`,
    ...
    `- **输入校验**：所有对外接口函数必须对输入做基本校验（类型、范围、空值），不得假设上游一定传入合法数据。尤其是构造核心数据结构的工厂函数，必须检验值域（如数字范围 1-9、数组长度 9 等），不能让不合法数据悄然进入系统。`,
    `- **UI 不得暴露内部标识符**：面向用户展示的文案不得直接使用代码级 ID（如技巧 id、错误码等），必须经过 i18n 映射或本地化表。若映射表缺失某项，fallback 不应是原始 ID，而应是通用的「未知」提示。`,
    `- **浅色/暗色主题一致性**：若项目有暗色主题支持，所有颜色样式必须同时指定 light 和 dark 变体（如 Tailwind 的 \`text-emerald-700 dark:text-emerald-400\`），不得只写一种模式的颜色。`,
    `- **.gitignore 维护**：创建运行时数据目录、测试输出目录或构建缓存时，必须同步将这些路径加入 .gitignore。`,
    ``,
    `测试编写注意事项：`,
